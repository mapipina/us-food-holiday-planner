        ...
        "meals": [
            {
                ...
                "strMeasure19": " ",
                "strMeasure20": " ",
                "strSource": "https://www.allrecipes.com/recipe/26303/syrian-spaghetti/",
                "strImageSource": null,
                "strCreativeCommonsConfirmed": null,
                "dateModified": "2025-10-14 21:41:48"
            },
            {
                "idMeal": "52770",
...
describe('Recipe Endpoints (GET /api/v1/recipes/:holidayId)', () => {
    beforeEach(() => {
        (Holiday.findByPk as jest.Mock).mockClear();
        (axios.get as jest.Mock).mockClear();
    });

    it('should return 200 and recipe details if holiday and recipe are found', async () => {
        (Holiday.findByPk as jest.Mock).mockResolvedValue(MOCK_MEAL);
        (axios.get as jest.Mock).mockResolvedValue(MOCK_RECIPE_RESPONSE);

        const response = await request(app).get(`/api/v1/recipes/${MOCK_HOLIDAY_ID}`);

        expect(response.statusCode).toBe(200);
        expect(response.body.status).toBe('success');
        expect(response.body.data.holiday.title).toBe(MOCK_MEAL.title);
        expect(response.body.data.recipes[0].idMeal).toBe(MOCK_RECIPE_RESPONSE.data.meals[0].idMeal);
    });

    it('should return 404 if the Holiday ID is not found in the database', async () => {
    ...
    it('should return 404 if the holiday is found but TheMealDB returns no recipes', async () => {
        (Holiday.findByPk as jest.Mock).mockResolvedValue(MOCK_MEAL);
        ...
        expect(response.statusCode).toBe(404);
        expect(response.body.status).toBe('fail');
        expect(response.body.message).toContain('No recipes found');
    });

    it('should return 500 if themealdb call fails', async () => {
        (Holiday.findByPk as jest.Mock).mockResolvedValue(MOCK_MEAL);
        ...
        expect(response.body.message).toContain('Failed to retrieve recipe data from themealdb.');
    });
});
